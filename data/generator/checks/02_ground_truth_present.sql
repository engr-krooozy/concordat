...
WITH all_txns AS (
  SELECT txn_id FROM `concordat-hack.bank_alpha.transactions`
  UNION DISTINCT SELECT txn_id FROM `concordat-hack.bank_meridian.transactions`
  UNION DISTINCT SELECT txn_id FROM `concordat-hack.bank_union.transactions`
)
