...
ASS_STYLE = (
    "Style: Default,Helvetica Neue,34,&H00FFFFFF,&H000000FF,&H00000000,&HB4000000,"
...
def still(src: Path, seconds: float, dest: Path, zoom: float = 1.08) -> None:
    ...
    run(
        [
            ...
            "-vf",
            (
                f"{FIT},zoompan=z='min(1+({zoom}-1)*on/{frames},{zoom})'"
...
def main() -> None:
    ...
    for key, src, kind, start in SHOTS:
        ...
        if kind == "still":
            ...
            have = [CONSOLE / f"{n}.png" for n in CONSOLE_ORDER if (CONSOLE / f"{n}.png").exists()]
            if not have:
    ...
    run(
        [
            ...
            "-1",
            "-i",
            str(A / "score-tension.wav"),
            "-stream_loop",
            "-1",
            "-i",
            str(A / "score-resolve.wav"),
            "-filter_complex",
            (
    ...
    run(
        [
            ...
            "-filter_complex",
            (
                "[0:a]loudnorm=I=-16:TP=-1.5:LRA=11[v];[v][1:a]amix=inputs=2:"
